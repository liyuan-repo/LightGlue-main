...
def plot_images(imgs, titles=None, cmaps="gray", dpi=100, pad=0.5, adaptive=True):
    ...
    n = len(imgs)  # image0=ndarray:(682,1024,3)  image1=ndarray:(682,1024,3)
    if not isinstance(cmaps, (list, tuple)):
        cmaps = [cmaps] * n  # cmaps=colormaps  cmaps = ['gray', 'gray']
    ...
    # fig代表画布(Figure)；ax代表这个绘图窗口上的坐标系区域(axes)，即笛卡尔坐标区
    # 函数原型subplot(nrows, ncols, index, **kwargs)，一般我们只用到前三个参数，
    # 将整个绘图区域分成 nrows 行和 ncols 列，而index用于对子图进行编号
...
def plot_matches(kpts0, kpts1, figs, color=None, lw=1.5, ps=4, a=1.0, labels=None):
    ...
    """
    # plt.gcf()与plt.gca():当前的图表和子图可以使用plt.gcf()和plt.gca()获得，分别表示
    # Get Current Figure 和 Get Current Axes
    # fig = plt.gcf()
    fig = figs
    ax = fig.axes
    ax0, ax1 = ax[0], ax[1]
    ...
    if isinstance(kpts0, torch.Tensor):  # isinstance可以用来检验变量是什么数据类型
    ...
    if lw > 0:
        for i in range(len(kpts0)):
            ...
            fig.add_artist(line)  # matplotlib库的axiss模块中的Axes.add_artist()函数用于将Artist添加到轴，然后返回Artist。
